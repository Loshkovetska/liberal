...
const Select = ({
  ...
  const closeSelect = (e: any) => {
    if (selectRef) {
      if (
        ...
        !e.target.classList.contains("auth-select")
      ) {
        setState({
          ...state,
          isSelectOpened: false,
  ...
  useEffect(() => {
    ...
    return () =>
      selectRef && document.removeEventListener("mousedown", closeSelect);
  });

  return (
    <div
      ref={selectRef}
      className={cn("select", state.isSelectOpened && "select--active")}
      onClick={(e) => {
      ...
      <div className="select__radios">
        {data.map((item: any, index: number) => (
          <input
            ...
            className="auth-select__radio"
          />
        ))}
      </div>
      {state.isSelectOpened && (
